...
function updateStats(){
  ...
  const animalBonus = active.length>0
    ? Math.round((active.reduce((s,a)=>s+(ANIMALS_DATA[a]?.speed||0),0)/active.length)*100)/100
    : 0;
...
function renderTable(){
  ...
  const FEAT = {
    buyCity:0, good:1, sellCity:2, buyPrice:3, sellPrice:4,
    profitPerUnit:5, profitPerTrip:6, time:7, profitPerMin:8
  };
  ...
  rows.forEach((r, idx) => {
    ...
    const hereMark = isHere && !isTop ? '<span class="here-marker" title="You are here">⌂</span>' : '';
...
function renderMobileCards(rows){
  ...
  if(!container) return;
  if(!rows.length){
    container.innerHTML='<div class="route-card" style="text-align:center;cursor:default;padding:16px"><span class="rc-empty-msg">⚔ No routes match your search ⚔</span></div>';
    return;
  }
  const ALPHA_SORTS = new Set(['good','buyCity','sellCity']);
  ...
  rows.forEach((r, idx)=>{
    ...
    if(r.returnObj){
      ...
      const rpm=ret.profitPerMin, rpu=ret.profitPerUnit;
      const rpmc=rpm>0?'profit':rpm<0?'loss':'zero';
      const rpuc=rpu>0?'profit':rpu<0?'loss':'zero';
      const rpt=ret.profitPerTrip, rptc=rpt>0?'profit':rpt<0?'loss':'zero';
    ...
    card.addEventListener('click',()=>{
      ...
      document.querySelectorAll('.route-card.card-open').forEach(c=>c.classList.remove('card-open'));
...
const GOODS_WITH_ICON = new Set(['Barley','Copper Ingot','Cotton Yarn','Dried Fish','Earthenware','Glassware','Iron Ingot','Leather','Linen','Olive Oil','Sea Salt','Tools','Weapons','Wheat','Wool','Coriander','Sesame','Saffron','Byzantine Silk','Persian Carpets']);
const GOODS_ICON_FILE = {'Coriander':'coriander','Sesame':'sesame','Saffron':'saffron','Byzantine Silk':'byzantinesilk','Persian Carpets':'persiancarpets'};

function goodIconHTML(good){
  if(!GOODS_WITH_ICON.has(good)) return '';
  const file = GOODS_ICON_FILE[good] || good;
  return `<img class="good-icon" src="icons/${file}.webp" alt="" loading="lazy">`;
}
...
function renderExpandCells(r){
  ...
  if (!ret) return `<td class="expand-td exp-first exp-last" colspan="11"><span class="expand-label">↩ Return Leg</span><span class="expand-empty">No profitable return cargo for this route.</span></td>`;
...
function renderEventsTab(){
  ...
  const legendHtml = Object.entries(EVENTS).map(([, v]) => {
    const down = v.dir < 0;
  ...
  const eventOptions = ['<option value="">Pick event</option>']
    .concat(Object.entries(EVENTS).map(([k,v])=>`<option value="${k}">${v.glyph} ${v.label}</option>`))
    .join('');
  ...
  for (const city of CITY_ORDER){
    ...
    grid.insertAdjacentHTML('beforeend', `<div class="${cls}" data-city="${city}">${innerHtml}</div>`);
  }
}

function evSetDur(city, mins, btn){
  document.getElementById('evdur-'+city).value = mins;
  btn.closest('.evrow-dur').querySelectorAll('.btn-preset').forEach(b => b.classList.remove('sel'));
...
function setPricesMode(mode){
  ...
  if (hint) hint.textContent = mode === 'buy'
...
function renderPricesTab(){
  ...
  for (const city of CITY_ORDER){
    ...
    parts.push(`<th class="pt2-th-city${isO?' pt2-origin-th':''}">${city}<span class="pt2-city-culture">${CITIES[city].culture}</span></th>`);
  }
  ...
  for (const good of GOODS){
    ...
    if (good.type !== curType){
      curType = good.type;
      parts.push(`<tr class="pt2-type-row"><td colspan="${1+CITY_ORDER.length}">${good.type}</td></tr>`);
    }

    parts.push(`<tr class="pt2-row"><td class="pt2-td-good" title="${good.name} · base $${good.base}">${good.name}</td>`);
    ...
    for (const city of CITY_ORDER){
      ...
      parts.push(`<td class="${cls}">${inner}${evHits?`<span class="pt2-event-dot dir-${evDir}"></span>`:''}</td>`);
    }
...
function buildAnimalSlots(count){
  ...
  const animalOpts = Object.keys(ANIMALS_DATA).map(a=>`<option value="${a}">${a}</option>`).join('');
...
['culture','religion','religionLevel','langLevel','backpack','extraStorage','autoWalk']
  .forEach(id=>document.getElementById(id).addEventListener('change',updateAll));
['caravanGamepass','byzantineRank','sassanidRank']
  .forEach(id=>document.getElementById(id).addEventListener('change',onCaravanChange));
...
function exportCSV(){
  if (!allRoutes.length){alert('No routes to export.');return;}
  const headers=['Buy City','Good','Type','Sell City','Buy Price','Sell Price','Profit/Unit','Profit/Trip','Time (min)','Profit/Min','Return Cargo'];
  const lines=[headers.join(',')];
  for(const r of allRoutes){
    const ret = r.returnObj ? `${r.returnObj.good} (+$${r.returnObj.profitPerUnit})` : 'None';
    lines.push([r.buyCity,r.good,r.goodType,r.sellCity,r.buyPrice,r.sellPrice,
      r.profitPerUnit,r.profitPerTrip,r.time,r.profitPerMin,ret].map(csvField).join(','));
  }
...
function stateToObj(){return getPlayerState();}
...
function applyState(s){
  ...
  if(s.saddlebags) s.saddlebags.forEach((sb,i)=>{
    const el=document.getElementById('saddle'+i);
    if(el) el.checked=!!sb;
  });
}

function autoSave(){ lsSet(LS_KEY, JSON.stringify(stateToObj())); }

const SETUPS_KEY='silkroad_setups';
function getSetups(){ return lsGetJson(SETUPS_KEY, {}); }
function saveSetups(obj){ lsSet(SETUPS_KEY, JSON.stringify(obj)); }
...
function renderBestLoop(){
  ...
  if (!card) return;
  if (!allRoutes.length){ card.innerHTML=''; return; }
...
function onCourierQuestChange(){
  document.getElementById('shortQuestDest').disabled = !document.getElementById('shortQuestCheck').checked;
  document.getElementById('longQuestDest').disabled  = !document.getElementById('longQuestCheck').checked;
...
function bestGoodForLeg(from, to, ps, slots){
  ...
  for (const good of GOODS){
    ...
    if (!best || profitPerTrip > best.profitPerTrip)
      best = {good:good.name, goodType:good.type, buyPrice, sellPrice, profitPerUnit, profitPerTrip, slots};
  }
  return best;
}
...
function runCourierPlanner(){
  ...
  if (!start)                       { out.innerHTML=`<div class="planner-empty">Pick a starting city first.</div>`; return; }
  if (!shortOn && !longOn)          { out.innerHTML=`<div class="planner-empty">Enable at least one courier quest.</div>`; return; }
  if (shortOn && !shortDest)        { out.innerHTML=`<div class="planner-empty">Pick a destination for the short quest.</div>`; return; }
  if (longOn  && !longDest)         { out.innerHTML=`<div class="planner-empty">Pick a destination for the long quest.</div>`; return; }
  if (shortOn && shortDest===start) { out.innerHTML=`<div class="planner-empty">Short quest destination can't be your starting city.</div>`; return; }
  if (longOn  && longDest===start)  { out.innerHTML=`<div class="planner-empty">Long quest destination can't be your starting city.</div>`; return; }
  ...
  packages.sort((a,b)=>(TRAVEL_TIMES[start]?.[a.dest]||99)-(TRAVEL_TIMES[start]?.[b.dest]||99));
  ...
  _courierData = {
    start, packages, outboundCities, outboundLegs, returnLegs,
    outboundTradeProfit, packageProfit, returnTradeProfit, totalProfit,
    deliveryMap, totalSlots, mustReturn, ps, phase:'outbound'
  };
  renderCourierUI(out, _courierData);
}

function renderCourierUI(out, data){
  const {start, packages, outboundLegs, returnLegs,
         outboundTradeProfit, packageProfit, returnTradeProfit, totalProfit,
         deliveryMap, mustReturn, ps, phase} = data;
  ...
  const pkgBreakdownHtml = packages.map(pkg=>{
  ...
  const tradeProfitShown = phase==='delivered'
    ? outboundTradeProfit + returnTradeProfit
    : outboundTradeProfit;
  const totalShown = phase==='delivered'
    ? totalProfit
    : outboundTradeProfit + packageProfit;

  const summaryHtml = phase==='delivered' ? `
  ...
  const deliverBtn = phase==='outbound' ? `
    <div class="courier-deliver-wrap">
      <button class="btn btn-gold" onclick="confirmCourierDelivery()">✔ Confirm Package Delivered</button>
    </div>` : (mustReturn && returnLegs.length>0 ? '' : `<div class="courier-complete-msg">✦ Journey complete — all packages delivered.</div>`);
...
function runOptimalFinder(){
  ...
  setTimeout(()=>{
      ...
      for (const r of religions){
        for (const f of faiths){
          for (const l of langs){
            const ps = {...baseState, culture:c, religion:r, religionLevel:f, langLevel:l};
            let routes = generateRoutes(ps);
            routes = enrichRoutes(routes, ps);
            // best round trip
            const byPair = {};
    ...
    if (!best){ out.innerHTML = `<div class="planner-empty">No profitable setup found.</div>`; return; }
...
function buildPriceBreakdown(route, kind){
  ...
  const rawCityMod = calculateCityModifier(good, city, ps.culture, ps.religion, ps.religionLevel, kind);
  const cityMod = rawCityMod;
  const langMod = calculateLanguageModifier(city, ps.culture, ps.langLevel, ps.religion, ps.religionLevel);
  ...
  const cityAmt    = cityMod ? Math.floor(good.base * Math.abs(cityMod)) * (cityMod>=0 ? modSign : -modSign) : 0;
  const langAmt    = langMod ? Math.floor(good.base * Math.abs(langMod)) * (langMod>=0 ? modSign : -modSign) : 0;
  const relAmt     = relMod  ? Math.floor(good.base * Math.abs(relMod))  * (relMod >=0 ? modSign : -modSign) : 0;
  const lines = [];
  lines.push(`<div class="bk-title">${kind==='buy'?'Buy':'Sell'} Price · ${good.name} in ${city}</div>`);
  lines.push(`<div class="bk-row"><span>Base price</span><b>$${good.base}</b></div>`);
  if (importCost>0) lines.push(`<div class="bk-row"><span>Imported (${importHops} hop${importHops!==1?'s':''} from producer)</span><b class="loss">+$${importCost}</b></div>`);
  if (distBonus>0)  lines.push(`<div class="bk-row"><span>Distance (${distHops} hop${distHops!==1?'s':''} from producer)</span><b class="profit">+$${distBonus}</b></div>`);
  const modCls = (amt) => kind==='buy' ? (amt<=0?'profit':'loss') : (amt>=0?'profit':'loss');
  const modFmt = (amt) => `${amt>=0?'+':''}$${amt}`;
  if (cityMod) lines.push(`<div class="bk-row"><span>City modifier</span><b class="${modCls(cityAmt)}">${modFmt(cityAmt)}</b></div>`);
  if (langMod) lines.push(`<div class="bk-row"><span>Language</span><b class="${modCls(langAmt)}">${modFmt(langAmt)}</b></div>`);
  if (relMod)  lines.push(`<div class="bk-row"><span>Religion</span><b class="${modCls(relAmt)}">${modFmt(relAmt)}</b></div>`);
  if (repMod)  lines.push(`<div class="bk-row"><span>Reputation rank 6</span><b class="profit">-$${Math.floor(good.base*repMod)}</b></div>`);
  ...
  if (evDelta){
    ...
    lines.push(`<div class="bk-row"><span>${def.glyph} Event: ${def.label} (${lvlLabel})</span><b class="${evDelta>0?'profit':'loss'}">${sign}$${Math.abs(Math.round(evDelta))}</b></div>`);
  }
  if (!importCost && !cityMod && !langMod && !relMod && !repMod && !distMod && !evDelta) lines.push(`<div class="bk-row"><span>No modifiers apply</span><b>-</b></div>`);
...
function copyRouteForDiscord(idx){
  ...
  const backpackLabels = {None:null, SmallSatchel:'Small Satchel', LargeSatchel:'Large Satchel', BasketBackpack:'Basket Backpack', FramePack:'Frame Pack'};
  ...
  const animalCounts = {};
  ps.animals.forEach(a => { if(a !== 'None') animalCounts[a] = (animalCounts[a]||0)+1; });
  const animalStr = Object.entries(animalCounts).map(([a,c]) => c>1 ? `${a} ×${c}` : a).join(', ');
  const saddlebagCount = ps.saddlebags.filter(Boolean).length;
  const animalParts = [animalStr, saddlebagCount ? `${saddlebagCount} saddlebag${saddlebagCount>1?'s':''}` : ''].filter(Boolean);
...
🔗 [silkroadcalc.eu](https://silkroadcalc.eu/)`;
  ...
  if (navigator.clipboard && window.isSecureContext){
    ...
    if (fallbackCopy(msg)) onSuccess(); else onFail();
  }
}
function showToast(text){
  ...
  showToast._tid = setTimeout(()=>{ t.className=''; }, 1800);
}
...
async function syncFromApi() {
  try {
    ...
    if (gr.ok) {
      ...
      if (d.length) {
        GOODS = d.map(g => ({name:g.name, base:g.base_price, type:g.type, hopPct:g.hop_pct, produced_in:g.produced_in}));
        changed = true;
        routeDataChanged = true;
      }
    }
    if (tr.ok)   { const d = await tr.json();   if (Object.keys(d).length) { TRAVEL_TIMES = d; changed = true; } }
    if (citr.ok) {
      const d = await citr.json();
      if (d.length) {
        const c={};
        for (const r of d) c[r.name]={culture:r.culture,language:r.language,hasFireTemple:r.has_fire_temple,traits:r.traits,produced:r.produced};
        CITIES=c;
        changed=true;
        routeDataChanged = true;
      }
    }
    if (evr.ok)  { const d = await evr.json();  if (d.length) { const em={}, lm={}; for (const e of d) { em[e.name]={dir:e.dir,label:e.name,glyph:e.glyph,goodTypes:e.good_types,goodNames:e.good_names,desc:e.description}; for (const l of (e.levels||[])) { if (!lm[l.level]) lm[l.level]={pct:l.pct,base:l.base_bonus,label:{}}; lm[l.level].label[e.name]=l.label; } } EVENTS=em; EVENT_LEVELS=lm; changed=true; } }
    if (tefr.ok) { const d = await tefr.json(); if (d.length) { TRAIT_EFFECTS = d.map(r => ({trait_name:r.trait_name,kind:r.kind,bonus:parseFloat(r.bonus),cond_type:r.cond_type,cond_value:r.cond_value})); changed=true; } }
    if (rpr.ok)  { const d = await rpr.json();  if (d.length) { RELIGION_PERKS = d.map(r => ({religion:r.religion,min_level:r.min_level,perk_type:r.perk_type,multiplier:parseFloat(r.multiplier)})); changed=true; } }
    ...
    if (cr.ok) {
      ...
      if (logs.length) {
        ...
        if (body && latest.entries && latest.entries.length) {
          body.innerHTML =
            '<ul>' + latest.entries.map(e => `<li>${escHtml(e)}</li>`).join('') + '</ul>' +
            (latest.thanks ? `<div class="wn-thanks">Special thanks: ${escHtml(latest.thanks)}</div>` : '');
        }
      }
    }
  } catch (_) {}
...
function pingSession() {
  ...
  if (!sid) { sid = crypto.randomUUID(); lsSet('srtc-session-id', sid); }
...
const FB_WEBHOOKS = {
  feedback:   "https://discord.com/api/webhooks/1498685411936178340/zvZ2bQGi3KXsbljh-SHZbZwiP8Xlm6wtHvFXmYQUrgFv2-FGtwWh_v6gpQRu022RT6c3",
  suggestion: "https://discord.com/api/webhooks/1498685636780232839/Unhj-myxfubtYeT2u_vREIAbQI_2SpBeywcpPZz-id6d6u6KKeiJzLcb2WR510jucJnN",
...
const FB_META = {
  feedback:   { title:"💬 New Feedback",    color:0xd4a843, placeholder:"Tell us what's on your mind…",                submit:"Send Feedback"   },
  suggestion: { title:"✨ New Suggestion",  color:0x9aef9a, placeholder:"What would you like to see added or improved?", submit:"Send Suggestion" },
  bug:        { title:"🐞 New Bug Report",  color:0xd96b5a, placeholder:"Describe the bug, what you expected, and how to reproduce it.", submit:"Send Bug Report" }
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if(!url) { setStatus('Unknown type.', 'err'); return; }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => { closeForm(); setStatus(''); submitBtn.disabled = false; }, 1800);
      } else {
        throw new Error();
      }
    } catch (err) {
